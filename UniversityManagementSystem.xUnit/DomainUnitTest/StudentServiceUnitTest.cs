...
namespace UniversityManagementSystem.UnitTest.DomainUnitTest
{
    public class StudentServiceUnitTest
    {
        ...
        {
            int id = 1;
            Student expectedStu = new Student() { FirstName = "Test" };
            ...
            Assert.Equal(expectedStu.FirstName, result.FirstName);
            _MockStudentRepository.Verify(repo => repo.GetStudentByIdAsync(id), Times.Once);
        }
        [Fact]
        ...
        public async Task AddStudentAsync_StudentExsits_ReturnMinusOne()
        {
            ...
            _MockStudentRepository.Setup(repo => repo.GetStudentByIdAsync(student.StudentId)).ReturnsAsync(new Student() { FirstName = "TEST" });
        ...
        public async Task DeleteStudentAsync_DeleteError_ReturnZero()
        {
            ...
            int expectedResult = 0;
            _MockStudentRepository.Setup(repo => repo.DeleteStudentAsync(id)).ReturnsAsync(0);
            _MockStudentRepository.Setup(repo => repo.GetStudentByIdAsync(id)).ReturnsAsync(new Student() { FirstName = "TEST" });
        ...
        public async Task GetAllStudentsAsync_GetSuccess_ReturnList()
        {
            ...
            {
                new Student() { FirstName="Test1" },
                new Student() { FirstName="Test2"}
            };
        ...
        [Fact]
        public async Task GetAllStudentsByMajorIdAsync_ReturnNull()
        {
            int id = 1;
            _MockStudentRepository.Setup(repo=>repo.GetAllStudentsByMajorIdAsync(id)).ReturnsAsync((IEnumerable<Student>)null);

            IEnumerable<Student> result = await _studentService.GetAllStudentsByMajorIdAsync(id);
            
            Assert.Null(result);
            _MockStudentRepository.Verify(repo => repo.GetAllStudentsByMajorIdAsync(id),Times.Once);
        }
        [Fact]
        public async Task GetAllStudentsByMajorIdAsync_ReturnList()
        {
            int id = 1;
            _MockStudentRepository.Setup(repo => repo.GetAllStudentsByMajorIdAsync(id)).ReturnsAsync(new List<Student>());
